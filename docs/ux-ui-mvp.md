...
> **Примечание:** этот документ описывает **целевую UX-концепцию** MVP. Фактическая реализация в `frontend/src/` может отличаться: нет отдельных экранов регистрации/SSO (`/auth/verify`), заявок на вступление в команду; вход — логин/пароль; загрузка файлов (аватар, обложки) реализована поверх концепции «URL в профиле». Актуальное покрытие — [`mvp-coverage.md`](mvp-coverage.md), API — [`../frontend/docs/api-contract.md`](../frontend/docs/api-contract.md).
...
## 10. Что реализовано во фронтенде

В React-приложении (`frontend/src/`) реализованы экраны MVP:

| Экран | Файл |
| --- | --- |
| Главная (новости, биржа знаний, лента) | `pages/DashboardPage.tsx` |
| Профиль, КРК, достижения | `pages/ProfilePage.tsx`, `UserProfilePage.tsx` |
| Команда и каталог команд | `pages/TeamPage.tsx`, `TeamsListPage.tsx`, `TeamDetailPage.tsx` |
| Рейтинги и лиги | `pages/RatingPage.tsx` |
| Челленджи | `pages/ChallengesPage.tsx` |
| События / календарь | `pages/EventsPage.tsx` |
| Check-in, спасения, голосование | `pages/ToolsPage.tsx` |
| Панель организатора | `pages/AdminPage.tsx` |
| Вход | `features/auth/LoginPage.tsx` |

Соответствие требованиям ТЗ — в [`mvp-coverage.md`](mvp-coverage.md). Обзор проекта — в [`project-overview.md`](project-overview.md).
